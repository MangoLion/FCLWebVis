...
const {get_next_available_server} = require("./load_balancer")
//TODO
...
module.exports = function (app) {
	...
	app.get('/', function (req, res, next) {
		res.render('index', {title: "Visualization and Processing Webservice", userData: req.user, messages: {danger: req.flash('danger'), warning: req.flash('warning'), success: req.flash('success')}});
	...
	app.post('/login',	passport.authenticate('local', {
		...
		var next_url = get_next_available_server();
		if (next_url != "")
			res.redirect(next_url + '?user=' + req.user[0].email.trim());
			//res.redirect('http://localhost:3000/?user=' + req.user[0].email.trim());
	});
...
passport.use('local', new  LocalStrategy({passReqToCallback : true}, (req, username, password, done) => {
	...
	loginAttempt().then(result=>console.log(result))
